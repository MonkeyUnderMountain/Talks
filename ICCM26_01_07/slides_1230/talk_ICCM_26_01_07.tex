...
\begin{frame}{Kawaguchi-Silverman Conjecture}
    ...
    \begin{center}
    \begin{tabular}{cll}
        \only<4->{\textbullet & \(\displaystyle\alpha_f(x) := \lim_{n\to\infty} h(f^n(x))^{\frac{1}{n}}\)                     
                    & \textcolor{magenta}{arithmetic, local} invariant at \(x\); \\}
        \only<5->{\textbullet & \(\displaystyle \delta_f := \lim_{n\to\infty} ((f^n)^*H \cdot H^{\dim X - 1})^{\frac{1}{n}}\) 
...
\begin{frame}{Main results}
    ...
    \only<1->{
        ...
        \begin{itemize}
            ...
            \only<5->{\alert{\item \(f\) has a Zariski dense orbit;}}
            \only<5->{\alert{\item \(\delta_f > \delta_g\);}}
            \only<5->{\alert{\item if \(Y\) is of picard number one, then \(\delta_g = 1\).}}
        \end{itemize}
    }
    ...
    \only<4>{
        ...
        \alert{ If \(\delta_f > \delta_g\), under some extra conditions, we have another fibration by dynamical Iitaka theory.}
    }
...
\begin{frame}{Main results}
    ...
    \alert<2>{
        ...
        \begin{itemize}
            \item \(\kappa_f(X,R_f) \geq 1\); \only<4->{ \alert<4>{\(\Longleftarrow\) criterion for toric bundles}}
            \item \(f^*R_f \equiv \delta_f R_f\). \only<5>{ \alert<5>{\(\Longleftarrow\) decomposition of cones \(+\) no rational curve on \(Y\)}}
        \end{itemize}
    }
...
\begin{frame}[c]{Further questions}
    ...
    \begin{itemize}
        \item weaken the conditions on \(Y\): can we just assume \(Y\) is normal projective?
        \item 
    \end{itemize}
