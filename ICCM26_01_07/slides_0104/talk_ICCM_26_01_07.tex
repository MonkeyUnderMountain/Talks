...
\begin{frame}{Kawaguchi-Silverman Conjecture}
    ...
    Fix an ample line bundle \(H\) and \(h\geq 1\) a height function associated to \(H\).
...
\begin{frame}{Known cases for KSC}
    ...
    Quasi-projective surfaces (assume DML)            \hfill [Wang23];\\
    Birational map on surfaces                        \hfill [Xie24];\\
    Smooth projective 3folds with \(\deg f>1\)        \hfill [Meng-Zhang23];\\}
    \vspace{1em}
...
\begin{frame}{Main results}
    ...
    \only<1-7>{
        % Settings:
        \textbf{Special Fano fibration case (*)} (as the finial output of MMP):\\
        {\fontsize{10pt}{12pt}\selectfont
            \(X \xrightarrow{\pi} Y\)  smooth Fano fibration of relative picard number (\textcolor{green!60!black}{eg. \(\bbP^n\)-bundles}) over 
            \vspace{-0.8em}
        ...
        \only<6-7>{\alert<6>{
            ...
            {\fontsize{10pt}{12pt}\selectfont
                The fibration \((X,f)\xrightarrow{\pi} (Y,g)\) does not preserve the dynamical degree: \(\delta_f > \delta_g\);\\
    ...
    \only<4>{
        After iteration, the the dynamics \(f \acts X\) descends to \(g \acts Y\).
    ...
    \only<7-8>{
        \begin{theorem}[{New Fibration, [Meng-Wang-Y]}]\label{thm: mainthm2}
            In the special Fano fibration case (*) with dynamical conditions (\(\dagger\)), there exists an equivariant fibrations
...
\begin{frame}{Proof of \ding{172}: \(f^*R_f \equiv \delta_f R_f\)}
    ...
    \only<1-8>{
        ...
        \begin{tikzpicture}[
            ...
            \visible<7->{\node[box, below=of star2] (positivity) {Positivity of abelian varieties: \\ have no rational curves.};}
...
\begin{frame}{Proof of \ding{173}: positivity of \(\kappa(X,R_f)\)}
    ...
    \begin{tikzpicture}[
        ...
        \visible<2->{
        \node[box, below right=0.25cm and 1.2cm of start] (logetale) {\(f\) étale outside \(D\)};
        }
